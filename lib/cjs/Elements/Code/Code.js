"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
var jsx_runtime_1 = require("react/jsx-runtime");
var react_syntax_highlighter_1 = require("react-syntax-highlighter");
var prism_1 = require("react-syntax-highlighter/dist/cjs/styles/prism");
var CodeComponent = function (_a) {
    var children = _a.children;
    return ((0, jsx_runtime_1.jsx)(react_syntax_highlighter_1.Prism, __assign({ language: "tsx", style: prism_1.okaidia, showLineNumbers: true, wrapLongLines: true }, { children: children }), void 0));
};
exports.default = CodeComponent;
